...
export class orders extends Component {
    constructor(props) {
        super(props)
        this.state = { time: '12:30 PM', date: '12 Jen 2022', open: false, status: 'placed', data: {}, products: {} }
    }

    componentDidMount() {
        try {
            const res = this.props.location.state[0];
            ...
            const data = JSON.stringify({ ids: Object.keys(res.items) })
            // console.log(data);
            axios.post(API_URL + 'products/cart-items', data, config)
                .then(response => {
                    // const res = response.data;
                    // console.log(response.data);
                    this.setState({ time: formatTime(res.createdAt), date: formatDate(res.createdAt), status: res.status, data: res, products: response.data });
                }).catch(error => {
                    if (error.response) {
    ...
    getSum = (productId, price) => {
        const sum = price * this.state.data.items[productId];
        total = total + sum;
        // console.log(total)
        return sum;
    }
    totalItems = () => {
        let items = 0;
        const res = this.props.location.state[0];
        const array = Object.values(res.items)
        console.log(array)
    ...
    render() {
        const { open, status, date, time, data, products, } = this.state;
            ...
            <section className='px-2 pt-24'>
                ...
                {(open && (window.screen.width > 640)) &&
                    <div className="bg-white ">
                        <div className='flex flex-col xl:flex-row justify-between'>
                            <ul className='pl-16 xl:w-1/2'>
                                <p className='font-serif font-bold p-4 '>Track Delivery Status</p>
                            ...
                            <ul className='pt-2'>
                                <div className="border-b-2 rounded-lg shadow-lg ">
                                    <p className='text-sm font-serif border-b-2 rounded-lg p-2'>Items Details</p>
                                    <div className='text-left pt-6 px-4 mt-4 '>
                                        {products.map((product) =>
                                            <li key={product._id} className='flex flex-row bg-gray-100 pt-2 justify-between items-center m-2 p-4 rounded-md shadow-md' >
                                                <div className='inline-flex items-center justify-between'>
                                                    ...
                                                    <p className='font-sans font-medium px-4'>{product.name}</p>
                                                </div>
                                                <div className='flex xl:flex-col items-center justify-between w-1/2 xl:w-auto'>
                                                    <p className='font-sans font-medium px-2 text-right'>Quantity : {data.items[product._id]}</p>
                                                    <p className='font-sans font-medium px-2 text-left'>Price : ${this.getSum(product._id, product.price)}</p>
                                                </div>
                                            </li>
                                        )}
                                        ...
                                        <div className='inline-flex m-2 font-sans'>
                                            <h1 className='font-bold text-sm text-center'>${data.totalgrand}</h1>
                                        </div>
                                    </div>
                                </div>
                                <div className="rounded-lg border-b-2 mt-4 shadow-lg">
                                    <p className='text-sm font-serif border-b-2 rounded-lg p-2'>Shipping Details</p>
                                    <div className='text-left p-2 px-4'>
                                        <p className=' font-bold'>{data.name}</p>
                                        <p className=' body-font'>{data.address}</p>

                                        <div className='inline-flex py-2 font-sans'>
                                            <h1 className='font-bold text-sm'>Phone number :</h1>
                                            <h1 className='text-sm px-2'>{data.phone}</h1>
                                        </div>
                                    </div>
                                </div>
                                <div className="relative border-b-2 rounded-lg my-4 shadow-lg">
                                    ...
                                    <div className='text-left p-2 px-4'>
                                        ...
                                        <li className='flex flex-wrap py-2 font-sans justify-between'>
                                            ...
                                            <h1 className='text-sm px-2 font-bold'>${data.totalgrand}</h1>
                                        </li>
                                    </div>
                                </div>
                            </ul>
                        ...
                        <div className='flex flex-row justify-center items-center p-4 pb-10'>
                            <button className='bg-gray-600 hover:bg-gray-800 px-4 py-2 text-white shadow-md hover:shadow-lg rounded-lg'>Go Back</button>
                        </div>
                    </div>
                }
                    ...
                    <div id='mobileMenu' className='relative'>
                        <div className="bg-gray-100 border-b-2 rounded-lg shadow-lg mt-4">
                            ...
                            <div className='text-left pt-6 px-4 '>
                                {/* <span className="bg-indigo-500 text-white px-3 py-1 mb-2 tracking-widest text-xs absolute left-0 top-0 rounded-bl">OrderID : OD{data._id}</span> */}
                                {products.map((product) =>
                                    <li key={product._id} className='flex flex-row pt-2 bg-gray-200 py-4 px-2 m-2 shadow-md border-b-2 rounded-lg py-4 justify-between items-center'>
                                        ...
                                        <div className='flex flex-col justify-start p-2 text-left'>
                                            <p className='inline-flex justify-center px-2 items-center font-sans font-medium'>{product.name}</p>
                                            <p className='font-sans font-medium px-2 text-left'>Quantity : {data.items[product._id]}</p>
                                            <p className='font-sans font-medium px-2'>Price: ${this.getSum(product._id, product.price)}</p>
                                        </div>
                                    </li>
                                )}
                                <div className='inline-flex py-2 font-sans'>
                                    <h1 className='font-bold text-sm'>${data.totalgrand}</h1>
                                </div>
                        ...
                        <div className='flex flex-row justify-center items-center p-4 pb-10'>
                            <button onClick={() => this.props.history.push('/')} className='bg-gray-600 hover:bg-gray-800 px-4 py-2 text-white shadow-md hover:shadow-lg rounded-lg'>Go Back</button>
                        </div>
                    </div>
                }
                ...
                {!open &&
                    ...
                    <div onClick={() => this.setState({ open: true })} id='desktopmenu' className='relative bg-gray-100 flex flex-row justify-between m-4 p-4 bg-white rounded-lg shadow-lg hover:shadow-xl'>
                        <span className="bg-indigo-500 text-white px-3 py-1 tracking-widest text-xs absolute left-0 top-0 rounded-bl">OrderID : OD{data._id}</span>
                        <div className="flex justify-center p-4 items-center">
                            ...
                            <div className='flex flex-col '>
                                ...
                                <p className='font-sans font-bold px-4 '>items : {this.totalItems(data.items)}</p>
                            </div>
                        </div>
                        ...
                        <div className='flex flex-col items-start m-4'>
                            <div className='flex items-center'>
                                ...
                                <h1 className='font-bold text-left text-sm uppercase px-2 '>Order {data.status}</h1>
                            </div>
                            <h1 className='text-left pl-4 pt-2 font-bold text-sm'>At : {time}</h1>
                            <h1 className='text-left pl-4 font-bold text-sm'>On : {date}</h1>
                        </div>
                    </div>
                }
            </section>
